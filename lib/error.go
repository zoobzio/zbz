...
// Error represents an HTTP error with its metadata for documentation
type Error struct {
	Status      int
	Name        string
	Description string
...
type ErrorManager interface {
	SetError(statusCode int, error *Error)
	GetError(statusCode int) *Error
...
func (e *zErrorManager) SetError(statusCode int, error *Error) {
	e.errors[statusCode] = error
}

// GetError retrieves the error response for a given status code
func (e *zErrorManager) GetError(statusCode int) *Error {

	...
	return &zDocs{
		...
		spec: &OpenAPISpec{
			...
			Info: &OpenAPIInfo{
				...
				Description: "API generated with ZBZ framework",
			},
...
// SetDescription updates the API description
func (d *zDocs) SetDescription(description string) {
	d.spec.Info.Description = description
}
...
func (d *zDocs) AddSchema(meta *Meta) {
	...
	createKey := strings.ToLower(meta.Name) + "_create_payload"
	d.spec.Components.RequestBodies[fmt.Sprintf("Create%sPayload", meta.Name)] = &OpenAPIRequestBody{
		Description: Remark.MightGet(createKey, fmt.Sprintf("Request body for creating a %s", meta.Name)),
		Required:    true,
		Content: &OpenAPIContent{
			ApplicationJSON: &OpenAPIApplicationJSON{
	...
	updateKey := strings.ToLower(meta.Name) + "_update_payload"
	d.spec.Components.RequestBodies[fmt.Sprintf("Update%sPayload", meta.Name)] = &OpenAPIRequestBody{
		Description: Remark.MightGet(updateKey, fmt.Sprintf("Request body for updating a %s", meta.Name)),
		Required:    true,
		Content: &OpenAPIContent{
			ApplicationJSON: &OpenAPIApplicationJSON{

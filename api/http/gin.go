...
func NewGinDriver(devMode bool) *GinDriver {
	...
	// Add recovery middleware for panics - this is Gin-specific
...
func (g *GinDriver) AddMiddleware(middleware func(RequestContext, func())) error {
	...
	zlog.Zlog.Debug("Added middleware")
...
func (g *GinDriver) Start(address string) error {
	...
	zlog.Zlog.Info("Starting Gin HTTP server", zlog.String("address", address))
	
	// Start server and block until it stops
	if err := g.server.ListenAndServe(); err != nil && err != http.ErrServerClosed {
		zlog.Zlog.Error("Failed to start HTTP server", zlog.Err(err))
		return err
	}
...
func (g *GinDriver) Stop() error {
	...
	zlog.Zlog.Info("Stopping Gin HTTP server")
...
func (g *GinDriver) LoadTemplates(templatesDir string) error {
	if templatesDir == "" {
		zlog.Zlog.Debug("No templates directory specified, skipping template loading")
		return nil
	}
	
	// Load all .tmpl files from the templates directory
	pattern := templatesDir + "/*.tmpl"
	g.engine.LoadHTMLGlob(pattern)
	
	zlog.Zlog.Debug("Loaded HTML templates", 
		zlog.String("pattern", pattern),
		zlog.String("templates_dir", templatesDir))

...
func (s *zSchema) AddMeta(meta *Meta) {
	zlog.Zlog.Debug("Adding meta to schema",
		zlog.Any("meta", meta))
...
func (s *zSchema) IsValidTable(name string) bool {
	...
	zlog.Zlog.Debug("Checking table validity", 
		zlog.String("table", name),
		zlog.Int("total_metas", len(s.metas)),
		zlog.Int("total_tables", len(s.schema.Tables)),
		zlog.Strings("available_tables", s.getTableNames()),
		zlog.Any("schema_metas", s.metas))

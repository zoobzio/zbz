...
func NewDatabase(driver DatabaseDriver, name, description string) Database {
	zlog.Zlog.Info("Creating database service", 
...
func (db *zDatabase) Execute(contract *MacroContract, params any) (*sqlx.Rows, error) {
	...
	if !exists {
		zlog.Zlog.Error("Macro not found", zlog.String("macro", contract.Macro))
	...
	if err != nil {
		zlog.Zlog.Error("Failed to interpolate macro", 
			zlog.String("macro", contract.Macro),
			zlog.Err(err))
		return nil, err
	}
	...
	zlog.Zlog.Debug("Executing SQL", 
		zlog.String("contract", contract.Name),
		zlog.String("sql", sql))
...
func (db *zDatabase) Prepare(contract *MacroContract) error {
	...
	zlog.Zlog.Debug("Prepared statement", 
		zlog.String("contract", contract.Name),
		zlog.String("sql", sql))
...
func (db *zDatabase) LoadMacros(macrosDir string) error {
	...
	db.macros = macros
	zlog.Zlog.Info("Loaded macros", zlog.Int("count", len(macros)))
	return nil
}

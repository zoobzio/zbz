...
func NewAuth0Auth(cache cache.Cache, config *AuthConfig) AuthDriver {
	...
	zlog.Zlog.Info("Created OAuth2 config", 
		zlog.String("client_id", oauth.ClientID),
		zlog.String("redirect_url", oauth.RedirectURL),
		zlog.String("auth_url", oauth.Endpoint.AuthURL),
		zlog.String("scopes", fmt.Sprintf("%v", oauth.Scopes)))
	...
	if err := auth.refreshJWKS(); err != nil {
		zlog.Zlog.Fatal("Failed to load JWKS", zlog.Err(err))
	}
...
func (a *auth0Auth) ValidateToken(tokenString string) (*AuthToken, error) {
	...
	token, err := jwt.ParseWithClaims(tokenString, &Auth0Claims{}, func(token *jwt.Token) (any, error) {
...
func (a *auth0Auth) GetLoginURL(state string) string {
	...
	zlog.Zlog.Debug("Generated OAuth URL", 
		zlog.String("url", url),
		zlog.String("redirect_url", a.oauth.RedirectURL),
		zlog.String("client_id", a.oauth.ClientID))
	return url
}
...
func (a *auth0Auth) ExchangeCodeForToken(code, state string) (*AuthToken, error) {
	...
	// Extract ID token (contains user claims) instead of access token
	idToken := token.Extra("id_token")
	if idToken == nil {
		return nil, errors.New("no ID token received")
	}
	...
	return a.ValidateToken(idTokenString)
}
...
func (a *auth0Auth) refreshJWKS() error {
	...
	zlog.Zlog.Debug("JWKS refreshed successfully", zlog.String("url", a.jwksURL))
	return nil
}

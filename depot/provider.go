...
// StorageProvider is deprecated - use DepotProvider interface from service.go
// This remains for compatibility with existing code
type StorageProvider = DepotProvider
...
// NewMemory creates a new memory-based depot contract
func NewMemory(config interface{}) *DepotContract {
	memProvider := &MemoryProvider{
		data:          make(map[string][]byte),
		subscriptions: make(map[SubscriptionID]*memorySubscription),
	}
	return NewSimpleContract("memory", memProvider)
}

// NewMemoryProvider creates a new memory-based storage provider (legacy)
func NewMemoryProvider(config interface{}) (DepotProvider, error) {

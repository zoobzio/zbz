...
func TestYAMLMarshalScoping(t *testing.T) {
	user := User{
		ID:           123,
		Name:         "John Doe",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Marshal with permissions using YAML
			data, err := cereal.YAML.Marshal(user, tt.permissions...)
			if err != nil {
			...
			if err := yaml.Unmarshal(data, &result); err != nil {
				t.Fatalf("Failed to parse YAML marshal result: %v", err)
			}

			// Check expected fields are present
			for _, field := range tt.expectedFields {
				if _, exists := result[field]; !exists {
					t.Errorf("Expected field '%s' to be present in YAML, but it was filtered out", field)
				}
			}
			...
			for _, field := range tt.forbiddenFields {
				if _, exists := result[field]; exists {
					t.Errorf("Expected field '%s' to be filtered out of YAML, but it was present: %v", field, result[field])
				}
			}
		})
	}
...
func TestYAMLUnmarshalScoping(t *testing.T) {
	tests := []struct {
		name               string
		inputYAML          string
		permissions        []string
...
compliance_id: "COMP-123"`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Unmarshal with scoping using YAML
			err := cereal.YAML.Unmarshal([]byte(tt.inputYAML), &user, tt.permissions...)
			if err != nil {
...
func TestYAMLRoundTrip(t *testing.T) {
	...
	// Marshal with limited permissions using YAML
	data, err := cereal.YAML.Marshal(original, "public", "read")
	if err != nil {
		t.Fatalf("YAML Marshal failed: %v", err)
	}
	...
	yaml.Unmarshal(data, &filtered)

	if len(filtered) != 3 {
		t.Errorf("Expected 3 fields in filtered YAML data, got %d: %v", len(filtered), filtered)
	}

	// Unmarshal back with same permissions
	var result User
	err = cereal.YAML.Unmarshal(data, &result, "public", "read")
	if err != nil {
		t.Fatalf("YAML Unmarshal failed: %v", err)
	}
	...
	if result.ID != original.ID || result.Name != original.Name || result.Email != original.Email {
		t.Errorf("YAML round trip failed: expected ID=%d, Name=%s, Email=%s; got ID=%d, Name=%s, Email=%s",

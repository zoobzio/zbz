...
// watcher implements FluxContract for universal provider-based storage watching
type watcher[T any] struct {
	...
	maxFileSize            *int64
	subscriptionID         universal.SubscriptionID
	mu                     sync.RWMutex
}
...
// startWatching begins cloud event watching through universal provider
func (w *watcher[T]) startWatching() error {
	// Subscribe to universal provider events for this key
	ctx := context.Background()
	resourceURI := universal.ResourceURI{URI: "config://" + w.key}
	
	subID, err := w.provider.Subscribe(ctx, resourceURI, func(event universal.ProviderChangeEvent) {
		w.handleCloudEvent(event)
	})
	if err != nil {
		return fmt.Errorf("failed to subscribe to universal provider events: %w", err)
	}

	w.subscriptionID = subID
	zlog.Debug("Started universal watching", 
...
// handleCloudEvent processes events from universal provider storage
func (w *watcher[T]) handleCloudEvent(event universal.ProviderChangeEvent) {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *watcher[T]) processCloudEvent(event universal.ProviderChangeEvent) {
	...
	// Load new content from universal provider
	ctx := context.Background()
	resourceURI := universal.ResourceURI{URI: "config://" + w.key}
	content, err := w.provider.Get(ctx, resourceURI)
	if err != nil {
	...
	zlog.Debug("Processed cloud event", 
		zlog.String("key", w.key),
		zlog.String("operation", event.Type))
}
...
func (w *watcher[T]) scheduleThrottledUpdate(event universal.ProviderChangeEvent) {
	duration := w.getThrottleDuration()
...
func (w *watcher[T]) Dismiss() error {
	...
	// Unsubscribe from universal provider events
	if w.subscriptionID != "" {
		ctx := context.Background()
		if err := w.provider.Unsubscribe(ctx, w.subscriptionID); err != nil {
			zlog.Warn("Failed to unsubscribe from universal provider events", 
				zlog.String("key", w.key),
				zlog.Err(err))
		}
	}
